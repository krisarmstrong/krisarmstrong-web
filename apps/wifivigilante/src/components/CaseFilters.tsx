...
const FilterSelect = memo(function FilterSelect({
  ...
  return (
    <div className="flex-grow sm:flex-grow-0">
      <label htmlFor={id} className={`block text-xs font-medium text-text-muted mb-1 ${srOnlyLabel ? 'sr-only' : ''}`}>{label}</label>
      <select
        id={id}
        value={value}
      ...
      >
        <option value="">{placeholder || `All ${label.replace('Filter by ', '').replace('s', '')}`}</option>
...
const CaseFilters = memo(function CaseFilters({
  sectors,
    ...
    if (sector && sector.id) {
      ...
      getSubsectors(String(sector.id))
        .then(subsectorsData => {
          ...
          setSubsectors(subsectorsData.map(s => ({ id: s.id, name: s.name, sector_id: s.sector_id })));
        })
        .catch(err => {
    ...
    <div className="p-4 sm:p-6 bg-surface-raised border border-surface-border rounded-lg shadow-md">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
        <FilterSelect
          ...
          options={sectors}
          onChange={(val) => { setSelectedSectorName(val); setSelectedSubsectorName(""); }}
          disabled={isLoading}
