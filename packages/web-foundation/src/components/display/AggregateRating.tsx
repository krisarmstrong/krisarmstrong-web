...
export const AggregateRating: React.FC<AggregateRatingProps> = ({
  ...
  const handleStarClick = async (rating: number) => {
    ...
    const hadPreviousRating = userRating !== null;
    const previousRating = userRating;

    setSubmitting(true);
    setErrorMessage(null);
    setHoveredStar(null); // Clear hover state immediately
    setUserRating(rating); // Optimistic update for instant feedback
    setStatusMessage('Saving…');

    try {
      const result = await ratingAPI.submitRating(itemId, itemType, rating);
      if (result && result.success) {
        // Show appropriate message based on whether this is new or updated rating
        if (hadPreviousRating) {
          if (previousRating === rating) {
            setStatusMessage('Rating confirmed!');
          } else {
            setStatusMessage(`Updated to ${rating} star${rating !== 1 ? 's' : ''}!`);
          }
        } else {
          setStatusMessage('Thanks for rating!');
        }
        setStats(result.stats); // Update stats directly from response
