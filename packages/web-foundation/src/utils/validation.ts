 ...
 * Provides comprehensive validation for user inputs across all applications
 */
...
export function validateUrl(url: string, fieldName: string = 'URL'): ValidationResult {
  if (!url || url.trim().length === 0) {
    return { isValid: false, error: `${fieldName} is not configured` };
  }
  ...
  try {
    ...
    if (!parsedUrl.protocol.startsWith('http')) {
      return { isValid: false, error: `Invalid ${fieldName} protocol` };
    }
